...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    return {
        title: lang === 'ne' ? 'सफलताका कथाहरू' : 'Success Stories',
        description: lang === 'ne' ? 'युरोपमा नेपालीहरूको सफलताका कथाहरू।' : 'Success stories of Nepalis in Europe.',
    };
}
...
export default async function StoriesPage({ params }: Props) {
    ...
    `)
        .eq('status', 'published')
        .eq('categories.slug', 'success-stories')
        ...
        <div className="space-y-8 py-8">
            <div className="section-header">
                <h1 className="text-3xl md:text-4xl font-bold text-[var(--primary-900)]">
                    {lang === 'ne' ? 'सफलताका कथाहरू' : 'Success Stories'}
                </h1>
                <p className="text-neutral-600 mt-2">
                    {lang === 'ne' ? 'युरोपमा प्रेरणादायी सफलताका कथाहरू' : 'Inspirational success stories from Europe'}
                </p>
            </div>
