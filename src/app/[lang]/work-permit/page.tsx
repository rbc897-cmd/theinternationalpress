...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    return {
        title: lang === 'ne' ? 'वर्क परमिट' : 'Work Permit',
        description: lang === 'ne' ? 'युरोपमा काम गर्ने अनुमति सम्बन्धी जानकारी।' : 'Information about working in Europe.',
    };
}

export default async function WorkPermitPage({ params }: Props) {
    const { lang } = await params;
    ...
    `)
        .eq('status', 'published')
        .eq('categories.slug', 'work-permit')
        ...
        <div className="space-y-8 py-8">
            <div className="section-header">
                <h1 className="text-3xl md:text-4xl font-bold text-[var(--primary-900)]">
                    {lang === 'ne' ? 'वर्क परमिट' : 'Work Permit'}
                </h1>
                <p className="text-neutral-600 mt-2">
                    {lang === 'ne' ? 'युरोपमा रोजगारी र वर्क परमिट सम्बन्धी जानकारी' : 'Information about employment and work permits in Europe'}
                </p>
            </div>
